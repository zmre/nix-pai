...
function getEventsFilePath(): string {
  ...
  const now = new Date();
  const pstDate = new Date(now.toLocaleString('en-US', { timeZone: 'America/Los Angeles' }));
  const year = pstDate.getFullYear();
  const month = String(pstDate.getMonth() + 1).padStart(2, '0');
  const day = String(pstDate.getDate()).padStart(2, '0');
...
function getAgentForSession(sessionId: string): string {
  try {
    ...
    if (existsSync(mappingFile)) {
      ...
      return mappings[sessionId] || '@assistantName@';
    }
  } catch (error) {
...
async function main() {
  try {
    ...
    // If this is a SubagentStop or Stop event, reset to primary agent
      ...
      setAgentForSession(sessionId, '@assistantName@');
    }
